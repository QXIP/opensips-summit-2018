...
(function( $ ){
	...
	$('#mu-event-counter').countdown('2018/05/01').on('update.countdown', function(event) {
...
/*
		var sched_template = '{{#schedule}}<div role="tabpanel" class="tab-pane fade mu-event-timeline {{active}}" id="{{ id }}"><ul>{{#items}}<li><div class="mu-single-event"><img src="{{ icon }}"><p class="mu-event-time">{{ time }}</p><h3>{{ description }}</h3><span>{{ subtitle }}</span><p style="font-size:12px;text-align:left;margin-top:10px">{{ text }}</p></div></li>{{/items}}</ul></div>{{/schedule}}';
...
/*
		var template = '{{#speakers}}<div class="mu-single-speakers"><img src="{{ image }}" alt="{{ name }}" style="width:200px;height:200px;"><div class="mu-single-speakers-info"><h3>{{ name }} <a href="https://twitter.com/{{ twitter }}" target="_blank" tabindex="-1"><i class="fa fa-twitter"></i></a></h3><p>{{ title }}</p><p>{{ presentation }}</p></div></div>{{/speakers}}';
